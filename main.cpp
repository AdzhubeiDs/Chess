            ...
            // KeyReleased чтобы не было 1000000 ходов при залипании клавиш
                ...
                if (e.key.code == Mouse::Right || e.key.code == Keyboard::Space) {
                    ...
                    if (rnm.bp == boardPieces::empt) {
                        ...
                        if (board[rnm.to1][rnm.to2] == -6 && rnm.to1 == 7) {
                            board[rnm.to1][rnm.to2] = -4; // ферзь
                            int x = 3, y = 0;
                            f[um[{rnm.to1, rnm.to2}]].setTextureRect(IntRect(intSize * x, intSize * y, intSize, intSize));
                        }
                ...
                // например соперник сделал невозможный ход
